/* Particle Drift — vanilla JS port (Originkit "Particle Drift")
   Raw WebGL1 (no Three.js needed): particles drift in a chosen direction,
   link to nearby particles and to the cursor. Recolored to the site theme. */
...
(function (global) {
  ...
  function initParticleDrift(containerId, overrides) {
    try {
      ...
      if (!lineProg || !dotProg) return null;
      ...
      let builtN = -1, builtW = 0, builtH = 0;
      let disposed = false;
      ...
      function render(now) {
        ...
        const density = Math.round(clampN(cfg.density, 10, 400));
        if (density !== builtN) {
        ...
        const hv = clampN(cfg.hover, 0, 200) / 100;
        const reach = 180 * (hv > 0 ? 1 : 0);
        const LINKD = clampN(cfg.linkDistance, 0, 400);
        const LINKW = clampN(cfg.linkThickness, 0.5, 8);

        const th = (clampN(cfg.direction, 0, 360) * Math.PI) / 180;
        const dirX = Math.sin(th);
        const dirY = Math.cos(th);

        let lines = 0;
        const pushLine = (x0, y0, x1, y1, a0, a1, mixv, wpx) => {
        ...
        for (let i = 0; i < nCount; i++) {
          ...
          const dx = ptr.x - nx[i], dy = ptr.y - ny[i];
        ...
        const cb = parseColor(cfg.baseColor, [0.2, 0.27, 0.36]);
        const ca = parseColor(cfg.accentColor, [0.22, 0.82, 1.0]);
        ...
        if (nCount > 0) {
          ...
          gl.uniform1f(u(dotProg, "uDot"), cfg.dotSize);
      ...
      const track = (e) => {
        ...
        ptr.x = ((e.clientX - r.left) / r.width) * (container.clientWidth || 1200);
        ptr.y = ((e.clientY - r.top) / r.height) * (container.clientHeight || 800);
      };
      ...
      // Decorative background only — must never block the rest of the page.
      return null;
    }
  }

  global.initParticleDrift = initParticleDrift;
})(window);
